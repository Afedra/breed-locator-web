...
from breed.breeds.views import BREEDS_NUM_PAGES, add
...
def network(request):
    ...
    return render(request, 'network.html', {'users': users})


@login_required
def profile(request, username):
    page_user = get_object_or_404(User, username=username)
    all_breeds = Breed.get_breeds().filter(user=page_user)
    paginator = Paginator(all_breeds, BREEDS_NUM_PAGES)
    breeds = paginator.page(1)
    from_breed = -1
    if breeds:
        from_breed = breeds[0].id
